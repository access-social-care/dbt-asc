...
All `mart_glos_*` tables are in production, serving the Gloucestershire LA data product. `mart_la_query_summary` (in `models/marts/analytics/`, `analytics` schema, relocated 2026-08 out of the RBAC-restricted `la_product` schema, see admin#5) is a cross-LA all-time aggregate (used for ad-hoc analysis). `helplines_advicepro_accessava` (in `models/staging/acs_helplines/`, `staging_asc_helplines` schema) reads from `stg_la_topic_mentions` and collapses to monthly UT1/UT2 grain — known consumer as of 2026-09: the `helplines_data` repo's State of the Nation (SOTN) build reads it directly via a Snowflake grant (see the model's own header comment for details). Do not restructure without checking with that pipeline's owner first.
